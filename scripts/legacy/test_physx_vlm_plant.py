"""
Test PhysX-Omni VLM with a plant-specific prompt that requests
fine-grained part decomposition: trunk, branches, leaves.
"""
import os
import sys
import argparse
...
PLANT_PROMPT = """\
Please analyze the given image of a plant and output its structured description in the following format (voxel grid=64):
Name: <plant name>
Category: Plant
...
IMPORTANT: Decompose the plant into its structural components. You MUST separate:
- Container/pot (if visible)
- Soil
- Main trunk/stem (the thick woody central structure)
- Individual major branches (each as a separate part)
- Leaf clusters or individual large leaves (each as a separate part)

For plant materials, use realistic physical properties:
- Trunk/stem: wood material, density ~0.6-0.8 g/cm^3, Young's modulus ~1.0-10.0 GPa, stiff
- Branches: wood material, density ~0.5-0.7 g/cm^3, Young's modulus ~0.5-5.0 GPa, semi-rigid
- Leaves: organic leaf material, density ~0.3-0.5 g/cm^3, Young's modulus ~0.001-0.01 GPa, flexible
...
Note: Movement types: A (Move freely), B (Slide), C (Rotate around axis), D (Rotate around point), CB (Rotate and Slide), E (Fixed to the world).
The container should be Fixed (E). Trunk should be fixed relative to container. Branches can rotate (C or D) relative to trunk. Leaves can move freely (A) relative to their branch."""
...
def main():
    ...
    parser.add_argument("--output_dir", type=str,
                        default="data/physx_vlm_output_plant")
    ...
    for img_path in args.images:
        ...
        print("  Step 1: Getting plant-specific structured description...")
